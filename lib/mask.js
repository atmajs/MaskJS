	...
	function parseAssertion(T, output) {
		...
		do {
			...
			switch (c) {
				...
				////	T.sliceToChar(c == 34 ? '"' : "'");
				////
				////	current[current.left == null ? 'left' : 'right'] = parseDirective(T.template.substring(start, T.index + 1));
				////
				////	++T.index;
				////	continue;
				// =
			...
			default:
				...
				//do {
				//	c = T.template.charCodeAt(++T.index);
				//} while (T.index < T.length && c !== 32 && c !== 33 && c !== 60 && c !== 61 && c != 62 && c !== 40 && c !== 41 && c !== 38 && c !== 124);
				//
				//current[current.left == null ? 'left' : 'right'] = parseDirective(T.template.substring(start, T.index));
				continue;
			}
		} while (1);
