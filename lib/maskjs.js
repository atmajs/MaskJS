...
window.mask = (function(w, Class) {
    ...
    var regexp = {
        ...
        '.# ': /[\.# ][^{]/g,
        // < :going through .class#id headers
        '>{;\'"': /[>\{;'"]/g,
        // < :going through attributes
        '[^\w]': /[^\w]/g,
        escapedChar: {
            "'": /\\'/g,
            '"': /\\"/g,
    ...
    var ICustomTag = Class({
        construct: function() {
            this.attr = {};
        },
        render: function(values, stream) {
            return stream instanceof Array ? Builder.buildHtml(this.noes, values, stream) : Builder.buildDom(this.nodes, values, stream);
        },
        _renderHtml: function(values, stream) {
            return Builder.buildHtml(this, values, stream);
        },
        _renderDom: function(values, container) {
            return Builder.buildDom(this.nodes, values, container);
        }
    }),
        CustomTags = (function(ICustomTag) {
            var List = Class({
                extends: ICustomTag,
    ...
    var Parser = {
        ...
        parseAttributes: function(T, node) {
            ...
            for (; T.index < T.length; T.index++) {
                ...
                case 59:
                case 62:
                    //console.log('exiting', T.template.substring(T.index));

                    if (_classNames != null) {
                        node.attr.class = _classNames.indexOf('#{') > -1 ? (T.serialize != null ? this.toFunction(_classNames) : {
                ...
                if (key != null) {
                    ...
                    if (value.indexOf('#{') > -1) {
                        value = T.serialize != null ? this.toFunction(value) : {
                            template: value
                        };
                    }
                    node.attr[key] = value;
        ...
        parse: function(T, nodes) {
            ...
            for (; T.index < T.length; T.index++) {
                ...
                case 39:
                    ...
                    if (content.indexOf('#{') > -1) content = T.serialize == null ? this.toFunction(content) : {
                        template: content
                    };
    ...
    return {
        ...
        renderDom: function(template, values) {
            if (typeof template === 'string') {
                var compiled = this.compile(template);
                return Builder.buildDom(compiled, values);
            }
            return Builder.buildDom(template, values);
        },
