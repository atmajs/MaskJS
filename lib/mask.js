...
// source ../src/7.Builder.old.js
function builder_build(nodes, values, container, cntx) {
	if (nodes == null) {
			return container;
		}
		...
		for (i = 0; i < length; i++) {
			...
			if (CustomTags.hasOwnProperty(node.tagName) === true) {
/* if (!DEBUG)
				try {
				*/
...
/* if (!DEBUG)
			...
			if (node.nodes != null) {
				builder_build(node.nodes, values, tag, cntx);
			}
