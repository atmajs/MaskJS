...
UTest({
	...
	'extractVars' (){
		...
		deepEq_(expression_varRefs('(var * var2 / (1 - same)) + object.fn(a.b.c)'),
			[
				'var',
				'var2',
				'same',
	...
	'array' (){
		...
		deepEq_(expression_eval('[[foo[a], foo.prop[0]], ("ba" + "z")]', {
			foo: { prop: 'x-foo' },
			a: 'prop'
		}), [
			['x-foo', 'x'], ['baz']
	...
	'object' (){
		...
		deepEq_(expression_eval("{ foo: prop, 'val': 5.5, }", { prop: 'baz'}),
			{
				foo: 'baz',
				val: 5.5
			}
	...
	'methods': {
		'should call controllers method and resolve property from controllers scope' () {
			...
			expression_eval('this.doSmth(foo, foo.substring(1))', {}, null, ctr);
		...
		'should call method on a statement' () {
			...
			var x = expression_eval("('fi' + date.getFullYear()).substring(1)", model);
			eq_(x, 'i2017');
		}
	}
});
