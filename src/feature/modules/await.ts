...
import { class_create, class_Dfr, is_Function } from 'atma-utils';
import { coll_each } from '@utils/coll';
import { log_error, reporter_createErrorNode } from '@core/util/reporters';
import { builder_resumeDelegate } from '@core/builder/util';
...
(function(){
	...
	var AwaitableRender = class_create(class_Dfr, {
		constructor: function(name, expression, nodes, model, ctx, container, ctr) {
			...
			Mask
				.renderAsync(node, model, (builder_Ctx as any).clone(ctx), null, ctr)
				.then(
					this.onComplete,
					this.rejectDelegate()
				);
