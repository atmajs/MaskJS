...
const DefineNode = class_create(Dom.Node, {
    ...
    stringify (stream) {
        ...
        if (args_ != null && args_.length !== 0) {
            str += ' (';
            str += toCommaSeparated(args_, get_arg);
            str += ')';
        }
        ...
        if (extends_ != null && extends_.length !== 0) {
            str += ' extends ';
            str += toCommaSeparated(extends_, get_compo);
        }
...
function toCommaSeparated(arr, getter) {
    var imax = arr.length,
        i = -1, str = '';
    while (++i < imax) {
        str += getter(arr[i]);
