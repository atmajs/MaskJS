...
window.mask = (function(w, d) {
	...
	var Parser = {
		...
		parse: function(T, nodes) {
			...
			for (; T.index < T.length; T.index++) {
				...
				case 59:
					/* ';' */
					/** conitinue if semi-column, but is not a single tag (else goto 125) */
					if (current.nodes != null) continue;
				case 125:
					/* '}' */
					if (current == null) continue;
	...
	return {
		/**
		 * @obsolete - will be exposed into seperate mask addon for using on server side, like node.js
		 * @see renderDom		 
